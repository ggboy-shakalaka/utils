package ggboy.java.common.utils.RSA;
...
/**
 * @author Mustry RSA2签名,加解密处理核心文件,注意:密钥长度2048
 * 
 */
public class BaseRSA {
	...
	public static byte[] sign(byte[] data, byte[] privateKey) throws CommonUtilException {
		try {
			...
			throw new CommonUtilException(ErrorCode.rsa_sign_error,e);
		}
	}
	...
	public static boolean verify(byte[] data, byte[] sign, byte[] publicKey) throws CommonUtilException {
		try {
			...
			throw new CommonUtilException(ErrorCode.rsa_verify_error,e);
		}
	}

	private static Signature getSignature(byte[] data, byte[] key, String wKey) throws CommonUtilException {
		try {
	...
	private static byte[] rsaAlgorithm(byte[] data, byte[] key, String wKey, int mode, int block) throws CommonUtilException {
		...
		try {
			...
			throw new CommonUtilException(ErrorCode.rsa_decrypt_error,e);
		} finally {
			if(out != null) 
